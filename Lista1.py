...
if np.any(X7 <= 0) or np.any(Y7 <= 0):
    print("Wartości w X lub Y są ujemne lub zerowe. Dodam 1 do wszystkich wartości.")
    X7 = np.where(X7 <= 0, 1, X7 + 1)  # Dodajemy 1 tylko do wartości <= 0
    Y7 = np.where(Y7 <= 0, 1, Y7 + 1)  # Dodajemy 1 tylko do wartości <= 0

# Transformacja danych (logarytm naturalny)
X_transformed7 = np.log(X7)
Y_transformed7 = np.log(Y7)

# Krok 3: Wyznaczenie prostej regresji dla przetransformowanych danych
slope_transformed, intercept_transformed, r_value_transformed, p_value_transformed, std_err_transformed \
    = stats.linregress(X_transformed7, Y_transformed7)
print(f'Współczynnik kierunkowy (przetransformowane): {slope_transformed}, wyraz wolny: {intercept_transformed}')

# Krok 4: Wizualizacja regresji dla przetransformowanych danych
Y_pred_transformed = intercept_transformed + slope_transformed * X_transformed7

plt.scatter(X_transformed7, Y_transformed7, label='Przetransformowane dane', color='blue')
plt.plot(X_transformed7, Y_pred_transformed, color='red', label='Regresja liniowa dla przetransformowanych danych')
plt.legend()
plt.title('Regresja liniowa dla przetransformowanych danych')
plt.xlabel('Log(X) (zmienna objaśniająca)')
